...
def kill_stale_multiprocess_helpers():
    """Kill leftover multiprocess spawn/resource_tracker processes from previous runs."""
    import os
    import signal
    import subprocess
    try:
        result = subprocess.run(
            ["pgrep", "-f", "from multiprocess.(spawn|resource_tracker) import"],
            capture_output=True, text=True
        )
        pids = result.stdout.strip().split()
        if pids and pids[0]:
            print(f"[cleanup] killing stale multiprocess helpers: {' '.join(pids)}")
            for pid in pids:
                try:
